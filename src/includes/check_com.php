<?php
	...
	if(isset($_POST['new_comment'])) {
		...
		if(isset($new_com_text)) {
			mysqli_query($conn, "INSERT INTO `parents` (`p_user`, `p_text`, `p_date`, `pcode`) VALUES ('$new_com_name', '$new_com_text', '$new_com_date', '$new_com_code')");
		}
		header("Location: ");
	}
	// new reply
	if(isset($_POST['new_reply'])) {
		...
		if(isset($new_reply_text)) {
			mysqli_query($conn, "INSERT INTO `children` (`ch_user`, `ch_text`, `ch_date`, `ch_parcode`) VALUES ('$new_reply_name', '$new_reply_text', '$new_reply_date', '$new_reply_code')") or die(mysqli_error());
		}
		header("Location: ");
	}
?>
